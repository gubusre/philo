...
int init_philos(t_data *data)
{
    ...
    while (i < data->n_philo)
    {
        ...
        p->is_full = 0;
        p->meals_eaten = 0;
        p->acquired_first_left = 0;

        /* inicializar meal_mutex */
        if ((ret = pthread_mutex_init(&p->meal_mutex, NULL)) != 0)
        {
            /* rollback: destruir meal_mutex ya creados y liberar */
            for (int j = 0; j < i; j++)
...
int init_data(t_data *data, int argc, char *argv[])
{
    ...
    if ((ret = pthread_mutex_init(&data->write_mutex, NULL)) != 0) return (1);
    ...
    {
        if ((ret = pthread_mutex_init(&data->forks[i], NULL)) != 0)
        {
            /* rollback: destruir los ya inicializados */
            for (int j = 0; j < i; j++)
